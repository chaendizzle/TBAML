...
public class InvRemoveToken extends Token
{
	String rawA;
	String rawB;
	...
	public InvRemoveToken(String rawA, String rawB, int depth, Compiler compiler)
	{
		...
		if (A.type != TokenType.INV)
		{
			throw new TBAMLCompileError(
					"left statement of 'add' expression '" + rawA + "' is a '" + A.type.name()
	...
	{
		// check inv
		((Inventory) A.evaluate(interpreter)).remove((String) B.evaluate(interpreter));
		return null;
	}
}
